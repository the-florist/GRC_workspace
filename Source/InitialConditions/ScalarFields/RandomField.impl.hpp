...
void RandomField::calc_spectrum()
{
    ...
    {
        // Putting the 0 mode in the right spot for PS calc
        int J = j;
        int K = k;
    ...
    for(int i=0; i<N/2+1; i++) for(int j=0; j<N; j++) for(int k=0; k<N; k++)
    {
        ...
        // If you're on a special axis or plane (excluing i > N/2)
        else if(i==0 || i == N/2)
        {
            // Special i-norm planes
        ...
        {
            // Special j-norm planes (excluding i > N/2 region)
